<div class="overflow-x-auto">
    <div class="flex w-full p-3 space-x-2">
        ...
        <div class="w-3/6">
            ...
            focus:outline-none focus:shadow-outline focus-ring-0 foucus:bg-indigo-50" placeholder="Search Employees...">
        </div>
        <!-- Order By -->
    ...
    <table class="w-full text-base text-left text-gray-700 dark:text-gray-400">
        ...
        <tbody>
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">                                    
                ...
                <td class="py-4 px-6">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium leading-4 bg-{{ $employee->status_color }}-100 text-{{ $employee->status_color }}-900 capitalize">
                        {{ $employee->status }}
                    </span>
                </td>
                <td class="py-4 px-6">
                    <a href="{{ route('employees.edit', $employee) }}" class="inline-flex items-center px-4 py-2 bg-yellow-400 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-500 focus:bg-yellow-700 active:bg-yellow-900 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 transition ease-in-out duration-150" >Edit</a>
    ...
    <div class="relative w-full h-full max-w-md md:h-auto">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-6 text-center">
                <form method="POST" action="{{ route('employees.destroy', 'employee_id') }}">
                    @csrf
                    @method('DELETE')
...
$(document).ready(function(){
    $('.viewBtn').on('click', function(){
        ...
        document.getElementById('view_type').innerHTML = data[2];
        document.getElementById('view_status').innerHTML = data[3];
        document.getElementById('view_email').innerHTML = data[4];
